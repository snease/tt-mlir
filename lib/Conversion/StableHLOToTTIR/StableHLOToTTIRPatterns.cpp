...
public:
  ...
  matchAndRewrite(mlir::stablehlo::ConstantOp srcOp,
    ...
    mlir::ElementsAttr valueAttr = rebuildValueAttr(srcOp.getValue());
...
private:
  ...
  // Rebuilding value of constant op for following tensors types.
  ...
  mlir::ElementsAttr rebuildValueAttr(mlir::ElementsAttr valueAttr) const {
    ...
    if (isa<IntegerType>(elementType)) {
      switch (bitWidth) {
      case 1: {
        return getElementAttr<bool>(valueAttr, 1);
      }
      case 8: {
        return getElementAttr<int8_t>(valueAttr, 8);
      }
      case 16: {
        return getElementAttr<int16_t>(valueAttr, 16);
      }
      case 32: {
        return getElementAttr<int32_t>(valueAttr, 32);
      }
      case 64: {
        return getElementAttr<int64_t>(valueAttr, 32);
      }
      default: {
  ...
  mlir::ElementsAttr getElementAttr(mlir::ElementsAttr valueAttr,
