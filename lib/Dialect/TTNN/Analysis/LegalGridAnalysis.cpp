...
bool mock_is_output_tensor_legal_for_op(Operation *op, TensorConfigAttr layout) {
  // Placeholder, needs to be replaced with a call the the TTNN op interface.
  return true;
}

bool tensor_shape_compatible_with_shard(Operation *op, TensorConfigAttr layout) {
...
bool LegalGridAnalysis::applyOverrides() {
  ...
  RankedTensorType tensorType =
      mlir::cast<RankedTensorType>(op->getResult(0).getType());
  TensorConfigAttr layout = mlir::cast<TensorConfigAttr>(tensorType.getEncoding());

  analysisResult.push_back(
      layout.withBufferType(op->getContext(), override.bufferType)
