...
bool TensorConfigAttr::hasShardedTensorMemoryLayout() const {
  assert(getMemLayout() && "Expected a tensor memory layout");
...
{
  SmallVector<int64_t> shape = isTiled()
                                   ? getTiledShape(tensorShape)
                                   : SmallVector<int64_t>(tensorShape);
  MemorySpace memorySpace = utils::toTTMemorySpace(getBufferType());
  AffineMap linearMap =
      isTiled() ? getIdentityTileLinearMap() : getLinear();
...
TensorConfigAttr TensorConfigAttr::withGrid(
  ...
  return get(context, tensorShape, getElementType(), getBufferType(), grid,
             collapseIntervals, getMemLayout());
}

// Construct a new TensorConfigAttr
//
// This function creates a new TensorConfigAttr with the given parameters.
// The shape of the tensor, buffer type, element type and memory layout are preserved.
//
// /param context The MLIR context.
...
TensorConfigAttr TensorConfigAttr::withGrid(
    ...
    ArrayRef<std::pair<std::int64_t, std::int64_t>> collapseIntervals) {
  assert(ty);
  return TensorConfigAttr::withGrid(context, ty.getShape(), grid, collapseIntervals);
}
...
TensorConfigAttr TensorConfigAttr::withElementType(::mlir::MLIRContext *context,
  ...
  return TensorConfigAttr::get(
      ...
      buildMemRef<BufferType, BufferTypeAttr>(context, getShardShape(), elementType, getBufferType()), getMemLayout());
}
...
TensorConfigAttr TensorConfigAttr::withBufferType(::mlir::MLIRContext *context,
  ...
  return TensorConfigAttr::get(
      context, getLinear(), getGrid(),
      buildMemRef<BufferType, BufferTypeAttr>(context, getShardShape(), getElementType(), memorySpace),
      getMemLayout());
}
...
TensorConfigAttr TensorConfigAttr::withMemoryLayout(::mlir::MLIRContext *context,
                                        TensorMemoryLayout memLayout) {
  return TensorConfigAttr::get(
      context, getLinear(), getGrid(),
      buildMemRef<BufferType, BufferTypeAttr>(context, getShardShape(), getElementType(), getBufferType()),
